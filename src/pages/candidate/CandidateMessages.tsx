...
export const CandidateMessages = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (assignment?.tutor_id) ids.add(assignment.tutor_id);
      adminRows?.forEach((r) => ids.add(r.user_id));

      if (ids.size === 0) { setTutors([]); return; }
  ...
  // Realtime: listen for ALL inbound messages so the candidate is never missed,
  // even if they haven't opened the conversation yet.
  useEffect(() => {
    if (!user) return;
    const ch = supabase
      .channel(`cmsg-inbox-${user.id}`)
      .on(
        "postgres_changes",
      ...
      <div className="border-r border-border/40 overflow-y-auto bg-gradient-to-b from-background/40 to-transparent">
        <div className="p-4 border-b border-border/40">
          <div className="text-xs uppercase tracking-[0.2em] text-primary/80 font-semibold flex items-center gap-2">
            <Sparkles className="w-3 h-3" /> Mentors
          </div>
        </div>
        {tutors.length === 0 && (
          <p className="p-4 text-sm text-muted-foreground italic">No mentors available yet.</p>
        )}
        {tutors.map((c) => (
          <button
            key={c.user_id}
            onClick={() => loadMsgs(c)}
            className={cn(
              "w-full text-left p-3 transition-all border-b border-border/20 flex items-center gap-3 group",
      ...
      <div className="md:col-span-2 flex flex-col bg-gradient-to-br from-background via-background to-primary/5">
        {!active ? (
          <div className="flex-1 flex flex-col items-center justify-center text-center p-8 gap-4">
            ...
            <div>
              <div className="font-serif text-2xl gold-text">Begin a Conversation</div>
              <div className="text-sm text-muted-foreground mt-1">Pick a mentor to start chatting.</div>
            </div>
          </div>
        ) : (
          <>
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-3">
              {msgs.length === 0 && (
                <div className="text-center text-sm text-muted-foreground py-12">
                  Say hello ✨ — your mentor is here for you.
                </div>
              )}
              {msgs.map((m, i) => {
